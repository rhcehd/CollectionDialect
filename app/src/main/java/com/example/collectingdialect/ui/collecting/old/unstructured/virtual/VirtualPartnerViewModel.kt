...
class VirtualPartnerViewModel: CollectingViewModel() {
    /*companion object {
        @JvmStatic
        @BindingAdapter("virtualPartnerArray", "virtualPartnerIndex", requireAll = true)
        fun setVirtualPartnerIndex(view: ImageView, array: Array<Int>, index: Int) {
    ...
    var imageIndex = 0
        ...
        set(value) {
            if(field != value) {
                field = value
                fileNamePrefix = "${userName}_virtual_partner_${currentSubject}_${imageIndex}"
                fileName = "${userName}_virtual_partner_${currentSubject}_${imageIndex}_${adapter.itemCount}"
